// compile with
// g++ -o ../../build/extract_averages_B3 extract_averages_B3.cpp `root-config --cflags --glibs` -Wl,--no-as-needed -lHist -lCore -lMathCore -lTree -lTreePlayer -lgsl -lgslcblas
...
main (int argc, char** argv)
{
  ...
  gDirectory->cd("MPPC B3 - 0.0-2.1/Crystal 9");


  //get w histo
  TCanvas *c_w = (TCanvas*) gDirectory->Get("W histogram - Crystal 9");
  TH1F* wHisto = (TH1F*) c_w->GetPrimitive("W histogram - Crystal 9");
  ...
  TH1F* basicCtr = (TH1F*) gDirectory->Get("Basic CTR - Crystal 9");
  ...
  gDirectory->cd("Plots");
  float neigh_ch[8] = {16,20,23,26,27,29,30,31};
  ...
  for(int i = 0 ; i < 8 ; i++)
  {
    ...
    sname << "T_Channel_" << neigh_ch[i] << " - T_Crystal_9";
  ...
  lightSharing.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC C2 for photoelectric event in crystal 9"))->GetMean() );
  lightSharing.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC C4 for photoelectric event in crystal 9"))->GetMean() );
  lightSharing.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC C3 for photoelectric event in crystal 9"))->GetMean() );
  lightSharing.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC A2 for photoelectric event in crystal 9"))->GetMean() );
  lightSharing.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC B2 for photoelectric event in crystal 9"))->GetMean() );
  lightSharing.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC A3 for photoelectric event in crystal 9"))->GetMean() );
  lightSharing.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC A4 for photoelectric event in crystal 9"))->GetMean() );
  lightSharing.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC B4 for photoelectric event in crystal 9"))->GetMean() );

  lightSharingErr.push_back( ((TH1F*) gDirectory->Get("Light collected in trigger crystal"))->GetMeanError() );
  lightSharingErr.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC C2 for photoelectric event in crystal 9"))->GetMeanError() );
  lightSharingErr.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC C4 for photoelectric event in crystal 9"))->GetMeanError() );
  lightSharingErr.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC C3 for photoelectric event in crystal 9"))->GetMeanError() );
  lightSharingErr.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC A2 for photoelectric event in crystal 9"))->GetMeanError() );
  lightSharingErr.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC B2 for photoelectric event in crystal 9"))->GetMeanError() );
  lightSharingErr.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC A3 for photoelectric event in crystal 9"))->GetMeanError() );
  lightSharingErr.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC A4 for photoelectric event in crystal 9"))->GetMeanError() );
  lightSharingErr.push_back( ((TH1F*) gDirectory->Get("Light collected on MPPC B4 for photoelectric event in crystal 9"))->GetMeanError() );
