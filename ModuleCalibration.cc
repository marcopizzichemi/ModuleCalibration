                    ...
                    if(comptonAnalysis)
                    {
                      ...
                      for(int iComptMppc = 0; iComptMppc < nmppcx ; iComptMppc++)
                      {
                        for(int jComptMppc = 0; jComptMppc < nmppcy ; jComptMppc++)
                        {
                              ...
                              //fit slices Y
                              comptonEnDepSumCharge->FitSlicesY(0, 0, -1, 0, "QNRG1S");
                  ...
                  if(CurrentCrystal->CrystalIsOn() /*| usingRealSimData*/) // save data only if the crystal was found
                  {
                    ...
                    if(comptonAnalysis)
                    {
                      ...
                      for(int iCompt = 0 ; iCompt < CurrentCrystal->GetNumOfComptonHisto(); iCompt++)
                      {
                        ...
                        C_spectrum->cd();
                        CurrentCrystal->GetComptonEnDepSumCharge(iCompt)->Draw("COLZ");
                        C_spectrum->Write();
                        delete C_spectrum;
                        ...
                        C_spectrum->cd();
                        CurrentCrystal->GetComptonEnDepSumChargeGraph(iCompt)->Draw("A*");
                        C_spectrum->Write();
                        delete C_spectrum;
                      }
                    }
