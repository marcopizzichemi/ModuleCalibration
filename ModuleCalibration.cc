...
int main (int argc, char** argv)
{
  ...
  for(int iModule = 0; iModule < nmodulex ; iModule++)
  {
    for(int jModule = 0; jModule < nmoduley ; jModule++)
    {
      ...
      {
        allModuleChID.push_back(iDet); //dummy...
      }
      ...
      for(int iMppc = 0; iMppc < nmppcx ; iMppc++)
      {
        for(int jMppc = 0; jMppc < nmppcy ; jMppc++)
        {
          ...
          if(mppc[(iModule*nmppcx)+iMppc][(jModule*nmppcy)+jMppc]->GetIsOnForModular())
          {
            ...
            for(unsigned int iDet = 0; iDet < detector.size(); iDet++)
            {
              if(detector[iDet].digitizerChannel == channel)
              {
                thisChannelID = iDet;
              }
            }
            // std::cout << channel << " " << thisChannelID << std::endl;

            // neighbour channels IDs
            ...
            std::vector<std::string> neighbourChannels;
            ...
            if(correctingSaturation)
            {
              ...
              for(unsigned int iNeigh = 0; iNeigh < neighbourChID.size(); iNeigh++)
              {
                ...
                tempString = var.str();
                neighbourChannels.push_back(tempString);
                var.str("");
              }
              ...
              for(unsigned int iAll = 0; iAll < allModuleChID.size(); iAll++)
              {
                ...
                std::string tempString;
                tempString = var.str();
                allChannels.push_back(tempString);
                var.str("");
              }
            ...
            var << thisChannel.c_str() << " >> " << sname.str();
            ...
            // for(unsigned int iDet = 0; iDet < allModuleChID.size()-1; iDet++)
            // {
            //   TriggerChannel << "max(ch" << detector[allModuleChID[iDet]].digitizerChannel << ",";
            //   parenthesisCounter++;
            // }
            // TriggerChannel << "ch" << detector[allModuleChID[allModuleChID.size()-1]].digitizerChannel;
            ...
            {
              TriggerChannel << "max(" << allChannels[iDet] << ",";
              parenthesisCounter++;
            }
            TriggerChannel << allChannels[allChannels.size()-1];
            for(unsigned int iDet = 0; iDet < parenthesisCounter; iDet++)
            {
              TriggerChannel << ")";
            }
            ...
            cut.str("");
            cut << TriggerChannel.str().c_str() << " == " << thisChannel.c_str();
            ...
            var << thisChannel.c_str() << " >> " << sname.str().c_str();
            ...
            // 1. wAllChannels      -> 0 = just neighbour channels
            //                      -> 1 = all channels in the module
            // 1. usingAllChannelsForEnergySpectra  -> 0 = just neighbour channels
            //                                      -> 1 = all channels in the module
