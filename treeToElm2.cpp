...
// compile with
// g++ -o ../build/treeToElm2 treeToElm2.cpp `root-config --cflags --glibs` -Wl,--no-as-needed -lHist -lCore -lMathCore -lTree -lTreePlayer -lgsl -lgslcblas
...
int main (int argc, char** argv)
{
  ...
  for(unsigned int i = 0 ;  i < crystal.size() ; i++)
  {
    ...
    {
      std::cout << crystal[i].number << " " << crystal[i].detectorChannel  << std::endl;
    }
  }
  ...
  for (long long int i=0;i<neventAnalysis;i++)
  {
    ...
    for(unsigned int iCry = 0 ;  iCry < crystal.size() ; iCry++)
    {
      ...
      if(crystal[iCry].FormulaAnalysis->EvalInstance()) // good events for this crystal
      {
        gammas++;
        std::cout << i << " "<<  crystal[iCry].number << std::endl;
      }

      if(crystal[iCry].accepted)
      {
        // FIXME is it general enough?
        // check detector channel. feb1 goes from 0 to 31, feb2 from 32 to 63. There can only
        // be coincindence between different febs...
        ...
        // {
        //   // get detectorChannel
        //   if(crystal[iCry].detectorChannel < 32)
        //   {
        //     first_feb = 1;
        //   }
        //   else
        ...
        //   }
        //
        //   // run on all other channles
        //   for(unsigned int jCry = 0 ;  jCry < crystal.size() ; jCry++)
        //   {
        //     if(iCry != jCry) // except the same one
        //     {
        //       if(crystal[jCry].FormulaAnalysis->EvalInstance()) // good events for this crystal
        //       {
        //         // get detectorChannel
        //         if(crystal[iCry].detectorChannel < 32)
        //         {
